...
void http_response_set_status(http_response_t *r, int status, const char *reason) {
    if (!r) return;
    r->status = status;
    r->reason = (char *)reason; // Store as literal, not heap allocated
}
...
void http_response_set_body(http_response_t *r, const char *data, size_t len) {
    ...
    snprintf(content_length_str, sizeof(content_length_str), "%zu", len);
...
int http_response_serialize(const http_response_t *r, buf_t *out) {
    ...
    for (int i = 0; i < r->header_count; i++) {
        ...
        int header_len = snprintf(header_line, sizeof(header_line), "%s: %s\r\n", 
                                  r->headers[i][0], r->headers[i][1]);
        if (buf_append(out, header_line, (size_t)header_len) < 0) return -1;
    }
    ...
    if (!has_date) {
        ...
        int date_len = snprintf(date_header, sizeof(date_header), "Date: %s\r\n", date_str);
        if (buf_append(out, date_header, (size_t)date_len) < 0) return -1;
    }
